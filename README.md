...
If there are any errors of the form `[lncli] rpc error: code = Unknown desc = server is still in the process of starting`, then wait for a bit and run `npm run docker:connect`
...
The boltz server was initially started automatically when the container launched. This was done by adding what follows as an entrypoint command (when building the docker image above). However, we describe starting `boltzd` service manually to avoid an error on M1 where container kept exiting.

```
# get shell access
docker exec -it boltz-backend bash

#then start boltz service
root@e71c65b55701:/# cd boltz-backend && ./bin/boltzd --configpath ./config.toml
```

One benefit of doing this manually is that we can see the (Boltz) server output/logs in the terminal instead of say using docker desktop. But as mentioned above, this can be automated to be part of the image's entrypoint.
