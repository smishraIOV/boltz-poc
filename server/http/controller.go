...
func Controller(s *Server) *mux.Router {
	...
	// POST
	r.Path("/payment").Methods(http.MethodPost, http.MethodOptions).HandlerFunc(wrapPost(createPayment, s))

	r.Use(corsMw)
	return r
}

func getSwapInfo(s *Server) (any, error) {
	return s.boltz.GetReverseSwapInfo()
}

func getPair(s *Server) (any, error) {
	return s.boltz.GetPair()
}

func getPayments(s *Server) (any, error) {
...
func checkHealthHandler(s *Server) (any, error) {
...
func wrap(action func(s *Server) (any, error), server *Server) (f func(http.ResponseWriter, *http.Request)) {
	return func(w http.ResponseWriter, r *http.Request) {
		response, err := action(server)
...
func wrapPost(action func(s *Server, r *http.Request) (any, error), server *Server) (f func(http.ResponseWriter, *http.Request)) {
	return func(w http.ResponseWriter, r *http.Request) {
